...
HealthKit is an Apple capability available to free Personal Teams, but the signing tool must request it and Apple's final profile must authorize it. Merely declaring `com.apple.developer.healthkit` in source is insufficient. The [HealthKit signing guide](docs/healthkit-signing.md) covers free Xcode provisioning and the [experimental Windows helper](tools/healthkit-signer/README.md). Its offline checks do not establish physical-device acceptance. CI also verifies privacy descriptions and the local import URL scheme in the built IPA, not just source files.
